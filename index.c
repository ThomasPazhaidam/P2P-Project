...
int main(int argc, char *argv[])
{
	...
	char data_recieved[100], peer_name[10], content_name[10], address[13], port_num[10];
	...
	while (1) {
		...
		// Handle request to seach for content 
			...
			for (i=0; i<10; ++i){
				if ((strcmp(registered_content[i].content, content)==0) && (content_tracking[i]==1)){
					placement = i;
					found = 1;
				}
			}
		...
		// Handle request to register content
			...
			for (i=0; i<10; ++i){
				if ((strcmp(registered_content[i].peer, peer)==0) && (strcmp(registered_content[i].content, content)==0) && content_tracking[i]==1){
					error_found = 1;
				}
			}
			if (error_found ==0){
